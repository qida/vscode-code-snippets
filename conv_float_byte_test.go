...
func TestBytes8ToFloat64(t *testing.T) {
	tests := []struct {
		...
		{"2 [8]byte 转成 float64", []byte{0xff, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, 1.26e-321},
		{"3 [8]byte 转成 float64", []byte{0xff, 0xff, 0xff, 0xff, 0x00, 0x00, 0x00, 0x00}, 2.1219957905e-314},
	}
	for _, tt := range tests {
...
func TestFloat64ToBytes8(t *testing.T) {
	tests := []struct {
		...
		{"2 [8]byte 转成 float64", 1.26e-321, []byte{0xff, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}},
		{"3 [8]byte 转成 float64", 2.1219957905e-314, []byte{0xff, 0xff, 0xff, 0xff, 0x00, 0x00, 0x00, 0x00}},
	}
	for _, tt := range tests {
